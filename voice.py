...
if target_language_input not in language_map:
    print(f"Invalid language: {target_language_input}. Defaulting to Hindi.")
    target_language_code = language_map['Hindi']
else:
    target_language_code = language_map[target_language_input]

# Ensure microphone is accessible
try:
    with mc as source:
        print("Speak now for language detection and translation...")
        MyText = recognize_speech(recog1, source)

    # Retry mechanism for no input
    if not MyText:
    ...
    if MyText:
        ...
        try:
            ...
            if text_to_translate is None:
                ...
                print(f"Playing the translated audio: {audio_file}")
